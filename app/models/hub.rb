class Hub < ActiveRecord::Base
  ...
  %w[ pages posts blogs recipes interviews hots ].each do |model_name|
  ...
  class << self
    def published_tree name
      with_slug(name).descendants.published_rset
    end

    def main_articles_hubs
      with_slug(:posts).descendants.published.nested_set
    end
  ...
  def parents_until_slug slug
    ancestors.reverse.inject([]) do |items, item|
      break items if item.has_slug? slug
      items << item
      items
    end
  end
